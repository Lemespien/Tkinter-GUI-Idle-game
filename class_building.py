...
class Building():
    ...
    def build(self, checkbox_container):
        ...
        if count >= len(self.cost):
            ...
            if self.count == 0:
                if self.resource.check_box is None:
                    self.resource.check_box = tk.Checkbutton(
                        checkbox_container, text=f"automate {self.resource.name}", variable=self.resource.automated, command=lambda: toggle_automated(self.resource, self.count))
                    self.resource.check_box.pack()
...
Gatherers_Hut = Building("Gatherers Hut",
                         Food,
                         cost={Food: 10, Wood: 15})
Hunters_Cabin = Building("Hunters Cabin",
                         Food,
                         cost={Food: 10, Wood: 15},
                         value=5)
Lumberjack_Hut = Building("Lumberjack_Hut",
                          Wood,
                          cost={
                              Food: 10
                          })
...
Water_Well = Building("Water Well",
                      Water,
                      cost={
                          Food: 5,
                          Wood: 20,
